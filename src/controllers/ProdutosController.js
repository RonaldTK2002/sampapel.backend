...
module.exports = {
  ...
  async update(request, response) {
    try {
      ...
      if (result != 0)
      return response.status(200).json({notification:"Produtos updated succesfully"});
      else return response.status(400).json({notification:"Produtos_id not found"})
    } catch (error) {
  ...
  async delete(request, response) {
    try {
      ...
      if (result != 0)
        ...
        return response.status(404).json({ notification: "Produtos_id not found" });
    } catch (error) {
